...
export class TaskStatusService {
  ...
  getTaskStatuses(): Observable<TaskStatus[]> {
    return this.httpClient.get<TaskStatus[]>(
      'http://localhost:9090/api/taskstatuses',
      { responseType: 'json' }
    );
  }
  ...
  getTaskStatusByTaskStatusID(TaskStatusID: number): Observable<TaskStatus> {
    return this.httpClient.get<TaskStatus>(
      'http://localhost:9090/api/taskstatuses/searchbytaskstatusid/' +
        TaskStatusID,
      { responseType: 'json' }
    );
  }
  ...
  deleteTaskStatus(TaskStatusID: number): Observable<string> {
    return this.httpClient.delete<string>(
      'http://localhost:9090/api/taskstatuses?TaskStatusID=' + TaskStatusID
    );
  }
}
